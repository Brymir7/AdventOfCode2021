...
int fuelcost_part2(int a, int *b, int length_b){
    int fuelcost1 = 0;
    for (int i = 0; i < length_b; ++i)
    {
        int distance =  abs(a - b[i]);
        for(int j = 1; j<=distance; ++j){
            fuelcost1 = fuelcost1 + j;
        }
    }
    return fuelcost1;
}
...
int binary_search_part2(int min, int max, int *a, int length_a)
{
    int middle = (min + max) / 2;
    int fuelcost1 = fuelcost_part2(middle,  a, length_a);
    int fuelcost2 = fuelcost_part2(middle+1, a, length_a);
    printf("Fuelcost 1: %i, Fuelcost 2: %i Middle: %i\n", fuelcost1, fuelcost2, middle);
    printf("min: %i max: %i \n", min, max);
    if ((max - min == 1)||(max-min == 0))
    {
    ...
    if (fuelcost1 < fuelcost2)
    {
        return binary_search_part2(min, middle, a, length_a);
    }
    else if (fuelcost2 < fuelcost1)
    {
        return binary_search_part2(middle+1, max, a, length_a);
    }
...
int main(int argc, char **argv)
{
    ...
    int best_position_part2 = binary_search_part2(0, max_value1, temporary_array, length_input);
    printf("%i \n", best_position);
    printf("%i \n", best_position_part2);
    int output_fuelcost = fuelcost(best_position, temporary_array, length_input);
    int output_fuelcost_part2 = fuelcost_part2(best_position_part2, temporary_array, length_input);
    printf("%i\n", output_fuelcost);
    printf("%i\n", output_fuelcost_part2);
    return 0;
}